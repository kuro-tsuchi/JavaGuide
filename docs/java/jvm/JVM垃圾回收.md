...
- [1. JVM 垃圾回收](#1-jvm-垃圾回收)
  - [1.1. 常见面试题](#11-常见面试题)
    - [1.1.1. 本文导火索](#111-本文导火索)
  - [1.2. 揭开 JVM 内存分配与回收的神秘面纱](#12-揭开-jvm-内存分配与回收的神秘面纱)
    - [1.2.1. 对象优先在 eden 区分配](#121-对象优先在-eden-区分配)
    - [1.2.2. 大对象直接进入老年代](#122-大对象直接进入老年代)
    - [1.2.3. 长期存活的对象将进入老年代](#123-长期存活的对象将进入老年代)
    - [1.2.4. 动态对象年龄判定](#124-动态对象年龄判定)
    - [1.2.5. 主要进行 gc 的区域](#125-主要进行-gc-的区域)
  - [1.3. 对象已经死亡？](#13-对象已经死亡)
    - [1.3.1. 引用计数法](#131-引用计数法)
    - [1.3.2. 可达性分析算法](#132-可达性分析算法)
    - [1.3.3. 再谈引用](#133-再谈引用)
    - [1.3.4. 不可达的对象并非非死不可](#134-不可达的对象并非非死不可)
    - [1.3.5. 如何判断一个常量是废弃常量？](#135-如何判断一个常量是废弃常量)
    - [1.3.6. 如何判断一个类是无用的类](#136-如何判断一个类是无用的类)
  - [1.4. 垃圾收集算法](#14-垃圾收集算法)
    - [1.4.1. 标记-清除算法](#141-标记-清除算法)
    - [1.4.2. 标记-复制算法](#142-标记-复制算法)
    - [1.4.3. 标记-整理算法](#143-标记-整理算法)
    - [1.4.4. 分代收集算法](#144-分代收集算法)
  - [1.5. 垃圾收集器](#15-垃圾收集器)

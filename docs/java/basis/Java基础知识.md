...
- [1. Java 基本功](#1-java-基本功)
  - [1.1. Java 入门（基础概念与常识）](#11-java-入门基础概念与常识)
    ...
    - [1.1.3. Java 和 C++的区别?](#113-java-和-c的区别)
    - [1.1.4. 为什么说 Java 语言“编译与解释并存”？](#114-为什么说-java-语言编译与解释并存)
  - [1.2. Java 语法](#12-java-语法)
    ...
    - [1.2.5. 自增自减运算符 (x++, x--)](#125-自增自减运算符-x-x--)
...
- [2. Java 面向对象](#2-java-面向对象)
  ...
  - [2.2. 面向对象三大特征](#22-面向对象三大特征)
    - [2.2.1. 封装](#221-封装)
      - [2.2.1.1. 封装的优点](#2211-封装的优点)
    - [2.2.2. 继承](#222-继承)
    - [2.2.3. 多态](#223-多态)
  - [2.3. 修饰符](#23-修饰符)
    - [2.3.1. java修饰符有哪些](#231-java修饰符有哪些)
    - [2.3.2. 在一个静态方法内调用一个非静态成员为什么是非法的?](#232-在一个静态方法内调用一个非静态成员为什么是非法的)
    - [2.3.3. 静态方法和实例方法有何不同](#233-静态方法和实例方法有何不同)
...
- [3. Java 核心技术](#3-java-核心技术)
  - [3.1. 反射机制](#31-反射机制)
    - [3.1.1. 3.1.1.静态编译和动态编译](#311-311静态编译和动态编译)
    - [3.1.2. 3.1.2.反射机制优缺点](#312-312反射机制优缺点)
    - [3.1.3. 3.1.3.反射的应用场景](#313-313反射的应用场景)
  - [3.2. 异常](#32-异常)
...
#### 1.1.4. 为什么说 Java 语言“编译与解释并存”？

高级编程语言按照程序的执行方式分为编译型和解释型两种。

1. 编译型语言是指编译器针对特定的操作系统将源代码一次性翻译成可被该平台执行的机器码；
2. 解释型语言是指解释器对源程序逐行解释成特定平台的机器码并立即执行。
...
注释并不会执行, 编译器在编译代码之前会把代码中的所有注释抹掉,字节码中不保留注释，是程序员写给自己看的
...
在编写程序的时候，需要大量地为程序、类、变量、方法等取名字，于是就有了标识符，简单来说，标识符就是一个名字。

有一些标识符，Java 语言已经赋予了其特殊的含义，只能用于特定的地方，这种特殊的标识符就是关键字。因此，关键字是被赋予特殊含义的标识符。
...
1. `==`: 它的作用是判断两个对象的地址是不是相等。即判断两个对象是不是同一个对象。
基本数据类型==比较的是值，引用数据类型==比较的是内存地址

    > 因为 Java 只有值传递，所以，对于 == 来说，不管是比较基本数据类型，还是引用数据类型的变量，其本质比较的都是值，只是引用类型变量存的值是对象的内存地址。

1. equals: 它的作用也是判断两个对象是否相等，它不能用于比较基本数据类型的变量。`equals()`方法存在于`Object`类中，而`Object`类是所有类的直接或间接父类。
...
new 运算符，new 创建对象实例，对象引用指向对象实例（对象实例在堆内存中, 对象引用存放在栈内存中）。一个对象引用可以指向 0 个或 1 个对象;一个对象可以有 n 个引用指向它。
...
继承是使用已存在的类作为基础建立新类的技术，新类的定义可以增加新的数据或新的功能，也可以用父类的功能，但不能选择性地继承父类。通过使用继承，可以快速地创建新的类，可以提高代码的重用，程序的可维护性，节省大量创建新类的时间 ，提高开发效率。
...
1. 访问控制修饰符
    ...
    1. default (即默认,可以为空): 在同一包内可见，不使用任何修饰符。使用对象：类、接口、变量、方法。
...
#### 2.3.2. 在一个静态方法内调用一个非静态成员为什么是非法的?
...
- **`Exception`** :程序本身可以处理的异常，可以通过 `catch` 来进行捕获。`Exception` 又可以分为 受检查异常(必须处理) 和 不受检��异常(可以不处理)。

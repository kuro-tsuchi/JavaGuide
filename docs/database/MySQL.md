...
- [4. 视频教程推荐](#4-视频教程推荐)
- [5. 常见问题总结](#5-常见问题总结)
  - [5.1. 什么是MySQL?](#51-什么是mysql)
  - [5.2. 存储引擎](#52-存储引擎)
    - [5.2.1. 一些常用命令](#521-一些常用命令)
    - [5.2.2. MyISAM和InnoDB区别](#522-myisam和innodb区别)
  - [5.3. 字符集及校对规则](#53-字符集及校对规则)
  - [5.4. 索引](#54-索引)
  - [5.5. 查询缓存的使用](#55-查询缓存的使用)
  - [5.6. 什么是事务?](#56-什么是事务)
  - [5.7. 事务的四大特性(ACID)](#57-事务的四大特性acid)
  - [5.8. 并发事务带来哪些问题?](#58-并发事务带来哪些问题)
  - [5.9. 事务隔离级别有哪些?MySQL的默认隔离级别是?](#59-事务隔离级别有哪些mysql的默认隔离级别是)
  - [5.10. 锁机制与InnoDB锁算法](#510-锁机制与innodb锁算法)
  - [5.11. 大表优化](#511-大表优化)
    - [5.11.1. 限定数据的范围](#5111-限定数据的范围)
    - [5.11.2. 读/写分离](#5112-读写分离)
    - [5.11.3. 垂直分区(待完善)](#5113-垂直分区待完善)
    - [5.11.4. 水平分区](#5114-水平分区)
  - [5.12. 解释一下什么是池化设计思想。什么是数据库连接池?为什么需要数据库连接池?](#512-解释一下什么是池化设计思想什么是数据库连接池为什么需要数据库连接池)
  - [5.13. 分库分表之后,id 主键如何处理？](#513-分库分表之后id-主键如何处理)
  - [5.14. 一条SQL语句在MySQL中如何执行的](#514-一条sql语句在mysql中如何执行的)
  - [5.15. MySQL高性能优化规范建议](#515-mysql高性能优化规范建议)
  - [5.16. 一条SQL语句执行得很慢的原因有哪些？](#516-一条sql语句执行得很慢的原因有哪些)
...
#### 5.2.2. MyISAM和InnoDB区别

5.5版之前, MyISAM是MySQL的默认数据库引擎。虽然性能极佳，而且提供了大量的特性，包括全文索引、压缩、空间函数等，但MyISAM不支持事务和行级锁，而且最大的缺陷就是崩溃后无法安全恢复。

5.5版本之后，MySQL引入了InnoDB（事务性数据库引擎），MySQL 5.5版本后默认的存储引擎为InnoDB。
...
MySQL 采用的是类似继承的方式指定字符集的默认值，每个数据库以及每张数据表都有自己的默认值，他们逐层继承。比如：某个库中所有表的默认字符集将是该数据库所指定的字符集

在数据的存储上，MySQL提供了不同的字符集支持。而在数据的对比操作上，则提供了不同的字符序支持。

字符集 DEFAULT_CHARACTER_SET_NAME, utf8, 
字符序 DEFAULT_COLLATION_NAME, utf8_general_ci
### 5.4. 索引
...
### 5.12. 解释一下什么是池化设计思想。什么是数据库连接池?为什么需要数据库连接池?
...
4.  **Twitter的雪花算法** ：Github 地址：https://github.com/twitter1. archive/snowflake。
5.  **美团的[Leaf](https://tech.meituan.com/2017/04/21/mt-leaf.html)分布式ID生成系统** ：Leaf 是美团开源的分布式ID生成器，能保证全局唯一性、趋势递增、单调递增、信息安全，里面也提到了几种分布式方案的对比，但也需要依赖关系数据库、Zookeeper等中间件。感觉还不错。美团技术团队的一篇文章：https://tech.meituan.com/2017/04/21/mt-leaf.html 。

### 5.14. 一条SQL语句在MySQL中如何执行的
